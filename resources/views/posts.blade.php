...
@section('container')
    ...
    <div class="row justify-content-center mb-3">
        <div class="col-md-6">
            <form action="/posts">
                @if (request('category'))
                <input type="hidden"name='category' valuse="{{ request('category') }}">

                @endif
                <div class="input-group mb-3">
                    <input type="text" class="form-control" placeholder="search.." name ="search" value="{{ request('search') }}">
                    <span class="input-group-text" type = "submit" >search</span>
                  </div>
            </form>
        </div>
    </div>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body text-center">
          ...
          <p>
            ...
            <a href="/posts?category={{ $posts[0]->category->slug }} " class ="text-decoration-none" > {{ $posts[0]->category->name  }}</a>{{ $post[0]->created_at->diffForHumans() }}
        </small>
        </p>
        ...
        @foreach ($post->skip(1) as $post)
            <div class="col-mmd-4 mb-3">
                <div class="card" style="">
                    <div class="position-absolute  px-3 py- 2 " style="background-color:rgba(0,0,0,5)">
